...
@date   2008-03-15
*/
class NetTrxClient : public Async::TcpClient
{
  public:
     ...
     */
    static NetTrxClient *instance(const std::string& remote_host,
    ...
    /**
     * @brief Send a UDP message over the connection
     * @param msg The message to send
     */
    void sendUdpMsg(NetTrxMsg::Msg *msg);
  ...
  protected:
     ...
     */
    NetTrxClient(const std::string& remote_host, uint16_t remote_port,
  ...
  private:
    typedef std::map<std::pair<const std::string, uint16_t>, NetTrxClient*>
      	    Clients;
    typedef enum
    {
    ...
    NetTrxClient(const NetTrxClient&);
    NetTrxClient& operator=(const NetTrxClient&);
    ...
    void sendMsgUdP(NetTrxMsg::Msg *msg);
